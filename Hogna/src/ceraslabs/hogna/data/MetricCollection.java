...
public class MetricCollection implements Serializable
{
	...
	private Map<String, Double>       m_mapMetricData        = new HashMap<>();
	private Map<String, List<String>> m_mapMetricToResources = new HashMap<>();
	private Map<String, List<String>> m_mapResourceToMetrics = new HashMap<>();
...
└────────────────────────────────────────────────┘
	...
	public String toString()
	{
		...
		Map<String, Double> copy = new TreeMap<>(this.m_mapMetricData);
		for (Entry<String, Double> entry : copy.entrySet())
		{
			String strResourceMetric = entry.getKey();
			double value = entry.getValue();
			
			sb.append(String.format("│ %-32s    %16.4f │\n", strResourceMetric, value));
		}
